...
exports.sharedProfileData = async function (req, res, next) {
  let viewerId
  try {
    viewer = jwt.verify(req.body.token, process.env.JWT_SECRET)
    viewerId = viewer._id
  } catch (e) {
    viewerId = 0
  }
...
exports.apiLogin = function (req, res) {
  ...
  user
    ...
    .then(function (result) {
      res.json({
        token: jwt.sign({ _id: user.data._id, username: user.data.username, avatar: user.avatar }, process.env.JWT_SECRET, { expiresIn: tokenLasts }),
